...
async function finalizarPedido() {
    ...
    const upsell = document.getElementById("upsell-adulto").checked;
    ...
    try {
        const res = await fetch("http://localhost:3000/api/checkout", {
            ...
            body: JSON.stringify({ nome, email, whatsapp, cpf, plano, upsell })
        });
        ...
        if (data.url) {
            ...
            alert("Erro ao redirecionar: " + data.erro);
        }
    } catch (error) {
